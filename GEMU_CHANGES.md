...
修改过的既有文件和新增文件可通过本源码树识别。本地转移验证包附带不含 ROM bytes 的工作树快照，但在构建期下载的三个 Meson 子项目和可访问的 GEmu fork revision 补齐前，不把该快照称为完整 corresponding source，也不据此公开分发 binary。本文件不是对各源码文件 SPDX 标识的替代；具体许可证仍以 `LICENSE`、`COPYING`、`COPYING.LIB` 和各文件头为准。
